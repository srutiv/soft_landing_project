...
class LanderODE2(Group):
    """for formulation 2: use problem 3 as a constraint for problem 4"""
    ...
    def setup(self):
        ...
        self.add_subsystem('eom', subsys=FlightDynamics2(num_nodes=nn),
                           promotes_inputs=['x', 'y', 'z', 'v_x', 'v_y', 'v_z', 'm', 'T_x',
                                            'T_y', 'T_z', 'Gamma', 'tf', 'res7b', 'res17a', 'res19'],
                           promotes_outputs=['xdot', 'ydot', 'zdot', 'v_xdot', 'v_ydot', 'v_zdot', 'mdot'])


class FlightDynamics2(ExplicitComponent):
    """
    ...
    def setup(self):
        ...
        self.add_input('tf', val=np.ones(nn), desc='final landing time tf', units='s')
        ...
        #self.add_output('res5a', val=np.ones(nn), desc='res5a rate undefined', units='m/s')
    ...
    def compute(self, inputs, outputs):
        ...
        # Objective
        # obj4 defined as the integral of Gamma(t) from 0 to tf
        obj4 = np.linalg.norm(Tc)
        ...
        """
        doesn't need to be included in promotes_inputs/promotes_outputs;
        intermediate quantity within FlightDynamics2 component used for computation;
        typically doesn't get passed between different components directly.
        """
        #outputs["obj4"] = obj4
        # constraint outputs
        # SHOULD BE DERIVATIVES WRT TIME, can be 0
        outputs["res5a"] = res5a
        outputs["res5b"] = res5b
        outputs["res7b"] = res7b
        outputs["res9a"] = res9a
